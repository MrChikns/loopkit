...
cd "$ROOT"
chmod +x scripts/git-hooks/pre-commit scripts/git-hooks/commit-msg scripts/git-hooks/pre-push scripts/leak-scan.sh 2>/dev/null || true
git config core.hooksPath scripts/git-hooks
echo "installed: core.hooksPath -> scripts/git-hooks (pre-commit + pre-push leak-scan, the"
echo "           commit-msg agent-session-trailer guard, plus the pre-push provenance gate"
echo "           for repos registered as a loopkit target — see AGENTS.md)"
