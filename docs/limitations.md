...
- **Both integration terminals re-gate the exact combined destination state.** The engineering
  ...
  (`packages/core/src/beats/dispatch.ts:5104`<!--cite:postIntegrationRegate-->). The target lane now
...
- **A claim is a lease, so a lagging live owner can still be picked over.** Every picking lane now
  *reserves* what it takes: the shared pick list defers to an already-active claim
  (`packages/core/src/beats/dispatch.ts:3793`<!--cite:queuedClaimDeference-->), which is a read, and all
...
- **The lifecycle is observed, but the detached process is not controlled.** A configured merge
  ...
  (`packages/core/src/fold.ts:895`<!--cite:foldDeploySucceeded-->).
  ...
  detached and unreferenced (`packages/core/src/beats/worktree-deps.ts:406`<!--cite:fireDeployOnMerge-->):
...
- **There is no automatic rollback.** A merge can carry a `certification.rollback` string, and the
  ...
  (`packages/core/src/fold.ts:819`<!--cite:certificationRollback-->). Nothing in the plane executes it —
...
- **~~A steered item can display one thing and build another.~~ Fixed — recorded here because this
  ...
  (`packages/core/src/fold.ts:1580`<!--cite:foldRespec-->) — the fields builders and the judge are
