...
- **Planning** (`packages/core/src/beats/dispatch.ts:2402`<!--cite:runPlanningLane-->) — runs *before* the
...
- **Target** (`packages/core/src/beats/dispatch.ts:3097`<!--cite:runTargetLane-->) — a targeted item is
  ...
  (`packages/core/src/beats/dispatch.ts:2863`<!--cite:targetPostIntegrationRegate-->).
...
(`packages/core/src/beats/dispatch.ts:501`<!--cite:claimArbitration-->, [ADR-007](decisions/ADR-007-claim-arbitration.md)).
...
(`packages/core/src/beats/dispatch.ts:3733`<!--cite:quotaDegraded-->), flips dispatch to collect-only for
...
(`packages/core/src/beats/dispatch.ts:3914`<!--cite:providerFallback-->), and a later successful
...
- ⚪ **Batch co-location is off by default.** Items per worktree defaults to
  **1**<!--pin:batchMaxItems-->. Raised above 1, dispatch deliberately pulls *overlapping*, small items
  ...
  (`packages/core/src/beats/dispatch.ts:3858`<!--cite:batchColocation-->). Overlap therefore has two
...
- **A pathology blocker that cannot merge does not strand the victim silently.** If the repair is missing
  ...
  (`packages/core/src/beats/reactor.ts:2216`<!--cite:blockedVictimTimeout-->).
...
(`packages/core/src/beats/dispatch.ts:1659`<!--cite:planScopedCommit-->) — anything else stays
...
(`packages/core/src/beats/dispatch.ts:1616`<!--cite:checkTouchesOverstep-->). The worker may also propose
the commit subject; dispatch uses it verbatim when present.

**The spine guard** (`packages/core/src/beats/dispatch.ts:1538`<!--cite:checkSpine-->) parks any diff
...
- 🔵 **`SPEC_SATISFIED` is now measured against the item's acceptance criteria, not its
  ...
  (`packages/core/src/judge.ts:153`<!--cite:judgeCriteriaBar-->). Grading free prose was the weak
...
- 🔵 A crashed or stalled worker has its uncommitted work captured as a salvage patch before the
  worktree is removed (`packages/core/src/beats/dispatch.ts:4606`<!--cite:salvageOnCrash-->), and the next
...
- The invariant: **no build reaches its destination without a gate covering every commit that landed
  ...
  (`packages/core/src/beats/dispatch.ts:5061`<!--cite:postIntegrationRegate-->).
...
- The push race is a *second*, later collision — master moved between the local merge and the push.
  ...
  (`packages/core/src/beats/dispatch.ts:5251`<!--cite:pushRaceReset-->), re-merges the approved branch and
  re-gates against the **fresh** base before retrying the push
  (`packages/core/src/beats/dispatch.ts:5277`<!--cite:pushRaceRegate-->). A dirty checkout stops the
...
- **The plane files its own bugs.** When the pathologist classifies a park as a plane infrastructure
  ...
  (`packages/core/src/beats/reactor.ts:2438`<!--cite:repairItemCapture-->). That never reaches your desk
...
- Running alongside on every autonomy-enabled reactor beat: orphaned-build detection,
  ...
  (`packages/core/src/beats/reactor.ts:3631`<!--cite:staleClaimReap-->), and a leaked-worktree sweep.
...
- 🔵 **A lone detached targeted build used to strand in `building` forever** — no gate, no merge, no
  ...
  (`packages/core/src/beats/dispatch.ts:3692`<!--cite:detachedTargetGuard-->, per
  ...
  (`packages/core/src/beats/dispatch.ts:3392`<!--cite:collectorSkipsTargets-->), leaving nothing
...
(`packages/core/src/beats/reactor.ts:4228`<!--cite:mustNeverAutoAccepts-->).
...
- **Plane health.** If the reactor beat, the dispatch beat or the instance probes are not affirmatively
  ...
  (`packages/core/src/beats/reactor.ts:3874`<!--cite:acceptWithholdKeys-->). **Unknown is not healthy** —
