...
- **`item.blocked` / `blockedOn` is not the scheduling DAG.** It is an older pathology repair
  ...
  **merges**
  (`packages/core/src/beats/reactor.ts:2210`<!--cite:blockedVictimRelease-->). The plane creates these
...
- **A pathology blocker that cannot merge does not strand the victim silently.** If the repair is missing
  ...
  (`packages/core/src/beats/reactor.ts:2304`<!--cite:blockedVictimTimeout-->).
...
- **The plane files its own bugs.** When the pathologist classifies a park as a plane infrastructure
  ...
  (`packages/core/src/beats/reactor.ts:2526`<!--cite:repairItemCapture-->). That never reaches your desk
...
- Running alongside on every autonomy-enabled reactor beat: orphaned-build detection,
  ...
  (`packages/core/src/beats/reactor.ts:3744`<!--cite:staleClaimReap-->), and a leaked-worktree sweep.
...
(`packages/core/src/beats/reactor.ts:4349`<!--cite:mustNeverAutoAccepts-->).
...
- **Plane health.** If the reactor beat, the dispatch beat or the instance probes are not affirmatively
  ...
  (`packages/core/src/beats/reactor.ts:3995`<!--cite:acceptWithholdKeys-->). **Unknown is not healthy** —
