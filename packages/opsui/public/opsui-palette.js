// Command-palette client module (WI-179). shell.js already opens/closes the palette
...
//   • navigate-on-activate for `navigate:<href>` actions (registry destinations)
...
(function () {
  ...
  // Reset filter + highlight whenever the palette is opened (shell.js removes [hidden]).
  function onOpen(root) {
    if (input(root)) input(root).value = '';
    applyFilter(root);
  }

  // Watch for shell.js toggling the [hidden] attribute so we can reset on open.
  var root = palette();
  ...
  document.addEventListener('keydown', function (event) {
    ...
    // Esc is handled by shell.js (closePalette).
  });
