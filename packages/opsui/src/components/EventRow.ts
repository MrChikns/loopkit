 ...
 *  link sweep (WI-349) so a WI-NNN id in the metadata row drills to its hub page — the
...
export type EventRowProps = {
  ...
  /** WI-180 origin chip — a small secondary StatusBadge shown after the state badge in the
...
export function EventRow(props: EventRowProps): string {
  ...
  const meta = props.metadata.length
    ? `<div class="opsui-eventrow__meta">` +
      ...
      `</div>`
    : '';
  // Item-hub link sweep (WI-349): D-NNN mentions inside a row's free-text summary (park
  // reasons, decision notes) link to the Knowledge page's decision anchor. linkifyDecisionRefs
  // escapes the text itself before inserting trusted anchor markup — a strict superset of the
  // old plain-esc rendering (rows with no D-NNN mention render byte-identical to before).
  const summary = props.summary
    ? `<p class="opsui-eventrow__summary">${linkifyDecisionRefs(props.summary)}</p>`
    : '';
