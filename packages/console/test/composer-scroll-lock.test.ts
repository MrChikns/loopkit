...
// (open → hidden, close → restored). Mirrors packages/opsui/test/composer-scroll-lock.test.ts —
// this console-shell.js copy carries the identical bug (and fix) for the legacy render path.
...
// Resolved relative to the compiled test's own location (dist-test/test/*.js) so it finds the
// package-root public/ dir under both `tsx`-style direct execution and the build:test pipeline.
const here = dirname(fileURLToPath(import.meta.url));
const jsPath = resolve(here, '../../public/console-shell.js');
