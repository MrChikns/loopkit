...
import { sampleLedger, hostileLedger, tieredMergeLedger } from './fixtures.js';
...
/** Scopes a rendered page down to one card's markup (between its title and the next card's
...
test('composer: the halted notice renders beside the intent box, and armed adds nothing', () => {
  ...
  const composerAt = halted.indexOf('opsui-topbar__intent');
  assert.ok(titleAt < statusAt && statusAt < composerAt, 'halted tag sits between the page title and global actions');
