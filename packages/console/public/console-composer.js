...
// preview chips with x-remove. Progressive enhancement — zero-JS path is a plain
// multipart submit via the "+ Attach" file label. Scoped to `form.opsui-composer` only.
(function () {
  'use strict';
  ...
  // Delegation — survives DOM patches.
  ...
  // After a capture round-trip the server redirects to ?captured=<id> so the confirmation
  // chip renders once. Strip it from the URL so a refresh or bookmark doesn't re-show a
  // stale "routing..." confirm for an old id.
  try {
